...
namespace SerialPortUtility
{
	...
	public class SerialPortUtilityPro : MonoBehaviour
	{
        ...
        private GetBreathPacket getBreathPacket;
        private void Awake()
        {
            getBreathPacket = GameObject.FindObjectOfType<GetBreathPacket>();
        ...
        public void ReadUpdate()
		{
			...
			if (size > 0)
			{
                ...
                //char[] stringDatab = System.Text.Encoding.UTF8.GetChars(dataArray, 0, iRet);
                //Debug.Log(dataArray[3] + ";" + dataArray[5]);

                getBreathPacket.GetThePacket(dataArray);
                if (EnableTrans)
				{
					switch (ReadProtocol)
					{
